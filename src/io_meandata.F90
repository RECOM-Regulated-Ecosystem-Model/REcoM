...
CASE ('O2f       ')
    if (use_REcoM) then
    call def_stream(nod2D,  myDim_nod2D,   'O2f',      'O2-flux into the surface water',  'mmolC/m2/d', GloO2flux(:), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
    end if
...
CASE ('REMOC          ')
   if (use_REcoM) then
   call def_stream((/nl-1, nod2D/),  (/nl-1, myDim_nod2D/),   'REMOC','Remineralization of DOC', 'mmolC/(m2*d)', Diags3D(:,:,29),          io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
   endif
CASE ('DISSOC          ')
   if (use_REcoM) then
   call def_stream((/nl-1, nod2D/),  (/nl-1, myDim_nod2D/),   'DISSOC','Dissolution of POC', 'mmolC/(m2*d)', Diags3D(:,:,30),          io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, mesh)
   endif
CASE ('otracers  ')
    do j=3, num_tracers
